...
![Version](https://img.shields.io/badge/version-5.9--debug--mix--logic-blue)
...
1. **Скачайте последнюю версию** со [страницы релизов](https://github.com/ваш-ник/DonutMatch/releases)
2. **Установите файлы** в соответствующие папки:
   - `donutmatch.smx` → `addons/sourcemod/plugins/`
   - `donutmatch.cfg` → `cfg/sourcemod/`
   - `donutmatch.inc` → `addons/sourcemod/scripting/include/`
3. **Создайте папку конфигов** → `cfg/donutmatch/`
4. **Настройте конфигурационные файлы** (см. ниже)
5. **Перезапустите сервер** или выполните `sm plugins load donutmatch`

## ⚙️ Конфигурация

### Основные настройки (автогенерируются)
```sourcemod
...
Создайте в папке `cfg/donutmatch/`:
...
donutmatch/
├── scripting/
│   └── donutmatch.sp     # Исходный код
├── plugins/
│   └── donutmatch.smx    # Скомпилированный плагин
├── include/
│   └── donutmatch.inc    # API для разработчиков
├── configs/
│   └── donutmatch.cfg    # Автогенерируемые настройки
└── cfg/
...
1. Проверьте [существующие issues](https://github.com/ваш-ник/DonutMatch/issues)
